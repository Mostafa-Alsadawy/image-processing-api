...
describe('Test error handling', () => {
    it('return empty array for good request', ():void => {
        expect(errors(new ImageModel('fjord', 300, 300))).toEqual([]);
    });
    it('return array with 1 entry for bad request with 1 wrong attribute', ():void => {
        expect(errors(new ImageModel('', 300, 300)).length).toEqual(1);
    });
    it('return array with 3 entry for bad request with 3 wrong attribute', ():void => {
